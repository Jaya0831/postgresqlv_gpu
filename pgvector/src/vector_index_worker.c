...
#if USE_GPU_CUVS
/*
 * Task data copied out of shared memory during the ring-buffer drain.
 * The embedded query_vector array ensures the data remains valid after the
 * ring lock is released and after the shmem slot is potentially recycled by
 * another backend.
 */
...
/*
 * GPU batch search dispatcher.
 *
 * Groups the collected tasks by index_relid (each has its own FlushedSegmentPool),
 * snapshots the segment pool once per group, determines per-query which segments
 * to search (filtering by each query's LSMSnapshot), then calls
 * BatchedConcurrentVectorSearchOnSegments which launches one Folly task per
 * segment and returns immediately.
 *
 * Queries with zero segments are handled inline: empty result written and
 * backend latch set before this function returns.
 */
static void
batch_vector_search(CollectedVectorSearchTask *tasks, int n_tasks)
{
    int   base;
    bool *processed = (bool *)calloc(n_tasks, sizeof(bool));

    for (base = 0; base < n_tasks; base++)
    {
        /* All declarations at the top of this block (C89 requirement) */
        Oid                   relid;
        int                   lsm_idx;
        ...
        int                  *group_indices;
        uint32_t              tail_snap;
        uint32_t             *seg_pool_indices;
        uint32_t              n_segs;
        uint32_t              seg_iter;
        int                  *query_seg_count;
        int                 **seg_query_lists;
        int                  *seg_query_cnt;
        uint32_t             *active_seg_s;
        int                   n_active_segs;
        int                  *gi_to_active;
        int                   n_active_queries;
        BatchQueryDescriptor *qdescs;
        int                   gi;
        uint32_t              s;
        int                   i;
        ...
        relid   = tasks[base].index_relid;
        lsm_idx = get_lsm_index_idx_no_loading(relid);

        /* Index not registered in the worker; notify all backends immediately */
        if (lsm_idx < 0)
        {
            for (i = base; i < n_tasks; i++)
            {
                if (!processed[i] && tasks[i].index_relid == relid)
                {
                    VectorSearchResult result = vs_search_result_at(tasks[i].backend_pgprocno);
                    PGPROC *client = &ProcGlobal->allProcs[tasks[i].backend_pgprocno];
        ...
        /* Collect all tasks for this relid into a group */
        group_size    = 0;
        group_indices = (int *)malloc(sizeof(int) * n_tasks);
        for (i = base; i < n_tasks; i++)
        {
            if (!processed[i] && tasks[i].index_relid == relid)
            {
        ...
        /* Snapshot segment pool; increment ref count once per segment */
        pthread_rwlock_rdlock(&pool->seg_lock);
        tail_snap        = pool->tail_idx;
        seg_pool_indices = (uint32_t *)malloc(sizeof(uint32_t) * MAX_SEGMENTS_COUNT);
        n_segs           = 0;
        seg_iter         = pool->head_idx;
        if (seg_iter != (uint32_t)-1)
        {
            do {
                if (pool->flushed_segments[seg_iter].in_used)
                {
                    atomic_fetch_add(&pool->flushed_segments[seg_iter].ref_count, 1);
                    seg_pool_indices[n_segs++] = seg_iter;
                }
                if (seg_iter == tail_snap) break;
                seg_iter = pool->flushed_segments[seg_iter].next_idx;
            } while (true);
        }
        pthread_rwlock_unlock(&pool->seg_lock);

        /* For each segment, determine which group queries need it */
        query_seg_count = (int *)calloc(group_size, sizeof(int));
        seg_query_lists = (int **)malloc(sizeof(int *) * n_segs);
        seg_query_cnt   = (int *)calloc(n_segs, sizeof(int));
        for (s = 0; s < n_segs; s++)
            seg_query_lists[s] = (int *)malloc(sizeof(int) * group_size);

        for (gi = 0; gi < group_size; gi++)
        {
            CollectedVectorSearchTask *task = &tasks[group_indices[gi]];
            for (s = 0; s < n_segs; s++)
            {
                FlushedSegment seg = &pool->flushed_segments[seg_pool_indices[s]];
                bool in_snapshot = false;
                int  j;
                for (j = 0; j < task->snapshot.scount && !in_snapshot; j++)
                {
                    in_snapshot = (task->snapshot.smt_ids[j] >= seg->segment_id_start &&
                                   task->snapshot.smt_ids[j] <= seg->segment_id_end);
                }
                if (!in_snapshot)
                    in_snapshot = (task->snapshot.gmt_id >= seg->segment_id_start &&
                                   task->snapshot.gmt_id <= seg->segment_id_end);
                if (!in_snapshot)
                {
                    seg_query_lists[s][seg_query_cnt[s]++] = gi;
                    query_seg_count[gi]++;
                }
            }
        }

        /* Segments no query needs: decrement ref count and drop from active list */
        active_seg_s  = (uint32_t *)malloc(sizeof(uint32_t) * n_segs);
        n_active_segs = 0;
        for (s = 0; s < n_segs; s++)
        {
            if (seg_query_cnt[s] == 0)
            {
                decrement_flushed_segment_ref_count(pool, seg_pool_indices[s]);
                free(seg_query_lists[s]);
            }
            else
            {
                active_seg_s[n_active_segs++] = s;
            }
        }

        /* Build BatchQueryDescriptor array; handle zero-segment queries inline */
        gi_to_active     = (int *)malloc(sizeof(int) * group_size);
        n_active_queries = 0;
        qdescs           = (BatchQueryDescriptor *)malloc(sizeof(BatchQueryDescriptor) * group_size);

        for (gi = 0; gi < group_size; gi++)
        {
            CollectedVectorSearchTask *task = &tasks[group_indices[gi]];
            if (query_seg_count[gi] == 0)
            {
            ...
            {
                gi_to_active[gi] = n_active_queries;
                qdescs[n_active_queries].query_vector = task->query_vector;
                qdescs[n_active_queries].topk         = task->topk;
                qdescs[n_active_queries].efs_nprobe   = task->efs_nprobe;
                qdescs[n_active_queries].dim          = task->dim;
                qdescs[n_active_queries].result_slot  = vs_search_result_at(task->backend_pgprocno);
                qdescs[n_active_queries].client_proc  = &ProcGlobal->allProcs[task->backend_pgprocno];
                n_active_queries++;
            }
        }

        /* Build SegmentQueryBatch array with remapped indices, then dispatch */
        if (n_active_queries > 0 && n_active_segs > 0)
        {
            SegmentQueryBatch *seg_batches =
                (SegmentQueryBatch *)malloc(sizeof(SegmentQueryBatch) * n_active_segs);
            int ai;

            for (ai = 0; ai < n_active_segs; ai++)
            {
                uint32_t       seg_s        = active_seg_s[ai];
                uint32_t       seg_pool_idx = seg_pool_indices[seg_s];
                FlushedSegment seg_data     = &pool->flushed_segments[seg_pool_idx];
                int            m            = seg_query_cnt[seg_s];
                int           *qidx         = (int *)malloc(sizeof(int) * m);
                int            n            = 0;
                int            qi;

                seg_batches[ai].segment.index_type  = seg_data->index_type;
                seg_batches[ai].segment.index_ptr   = seg_data->index_ptr;
                seg_batches[ai].segment.bitmap_ptr  = seg_data->bitmap_ptr;
                seg_batches[ai].segment.vec_count   = seg_data->vec_count;
                seg_batches[ai].segment.map_ptr     = seg_data->map_ptr;
                seg_batches[ai].segment.segment_idx = seg_pool_idx;
                seg_batches[ai].pool_seg_idx        = seg_pool_idx;

                for (qi = 0; qi < m; qi++)
                {
                    int active_idx = gi_to_active[seg_query_lists[seg_s][qi]];
                    if (active_idx >= 0)
                        qidx[n++] = active_idx;
                }
                seg_batches[ai].n_queries     = n;
                seg_batches[ai].query_indices = qidx; /* freed inside BatchedConcurrentVectorSearchOnSegments */

                free(seg_query_lists[seg_s]);
            }

            BatchedConcurrentVectorSearchOnSegments(qdescs, n_active_queries,
                                                    seg_batches, n_active_segs, pool);
            free(seg_batches);
        }
        else
        {
            /* No active segments: release the ref counts we incremented */
            int ai;
            for (ai = 0; ai < n_active_segs; ai++)
            {
                uint32_t seg_s = active_seg_s[ai];
                decrement_flushed_segment_ref_count(pool, seg_pool_indices[seg_s]);
                free(seg_query_lists[seg_s]);
            }
        }
        ...
        free(seg_query_cnt);
        free(seg_query_lists);
        free(active_seg_s);
        free(seg_pool_indices);
        free(group_indices);
    }
...
#if USE_GPU_CUVS
        /*
         * GPU batch path: sleep 1ms outside the ring lock so that multiple
         * backends have a chance to enqueue their tasks before we drain.
         * The sleep is intentionally short; a single-query workload will
         * still be dispatched after at most 1ms of extra latency.
         */
        pg_usleep(1000); /* 1ms batch accumulation window */

        {
            CollectedVectorSearchTask *gpu_tasks =
                (CollectedVectorSearchTask *)malloc(
                    sizeof(CollectedVectorSearchTask) * ring_buffer_shmem->ring_size);
            int n_gpu_tasks = 0;

            /*
             * Drain ALL tasks from the ring under one lock acquisition.
             * VectorSearch tasks are copied out for batching; maintenance
             * tasks are dispatched immediately via submit_maintenance_task
             * (which returns quickly, keeping the lock hold short).
             */
            LWLockAcquire(ring_buffer_shmem->lock, LW_EXCLUSIVE);
            while (ring_buffer_shmem->count > 0)
            {
                int       idx       = ring_buffer_shmem->head;
                TaskSlot *task_slot = vs_task_at(idx);

                if (task_slot->type == VectorSearchTaskType)
                {
                    CollectedVectorSearchTask *ct;
                    VectorSearchTask src = vs_search_task_at(idx);
                    Assert(n_gpu_tasks < ring_buffer_shmem->ring_size);
                    ct = &gpu_tasks[n_gpu_tasks++];
                    ct->index_relid       = src->index_relid;
                    ct->backend_pgprocno  = src->backend_pgprocno;
                    ct->topk              = src->topk;
                    ct->efs_nprobe        = src->efs_nprobe;
                    ct->dim               = (int)src->vector_dim;
                    ct->snapshot          = src->snapshot;
                    memcpy(ct->query_vector,
                           vs_search_task_vector_at(src),
                           sizeof(float) * src->vector_dim);
                }
                else
                {
                ...
                ring_buffer_shmem->head =
                    (ring_buffer_shmem->head + 1) % ring_buffer_shmem->ring_size;
                ring_buffer_shmem->count--;
            }
            LWLockRelease(ring_buffer_shmem->lock);

            if (n_gpu_tasks > 0)
                batch_vector_search(gpu_tasks, n_gpu_tasks);

            free(gpu_tasks);
        }
#else
        /* CPU path: original single-task drain loop */
        for (;;)
        {
            TaskSlot *task_slot;
