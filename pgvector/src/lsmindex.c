...
load_lsm_index(Relation index, uint32_t slot_idx)
{
    ...
    // By the time we reach here, the slot should be registered (valid == 2) with the correct Oid.
    ...
    {
        // TODO: for debugging
        elog(DEBUG1, "[load_lsm_index] In recovery step 2, checking segments that include sid in sids array");
        ...
        while (f_idx >= 0 && s_idx < num_sids)
        {
            ...
            {
                // TODO: for debugging
                elog(DEBUG1, "[load_lsm_index] In recovery step 2, processing segment %u-%u", files[f_idx].start_sid, files[f_idx].end_sid);
                SegmentId start_sid_disk, end_sid_disk;
                uint32_t valid_rows;
                IndexType seg_index_type;
...
bulk_delete_lsm_index(Relation index, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state)
{
    ...
    int lsm_idx = get_lsm_index_idx(index);
    if (lsm_idx < 0)
    {
        return stats;
    }
