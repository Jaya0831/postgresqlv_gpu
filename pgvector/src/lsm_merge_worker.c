...
claim_merge_task(int worker_id, int lsm_idx, uint32_t segment_idx0, uint32_t segment_idx1, int task_type)
{
    ...
    if (task_type == SEGMENT_UPDATE_MERGE)
    {
        ...
        {
            // elog(DEBUG1, "claim_merge_task: invalid segment indices for merge operation - worker %d, lsm_idx = %d, segment_idx0 = %u (valid: %s), segment_idx1 = %u (valid: %s)", 
            //      worker_id, lsm_idx, segment_idx0, 
            //      (segment_idx0 >= 0 && segment_idx0 < MAX_SEGMENTS_COUNT) ? "yes" : "no",
            //      segment_idx1, (segment_idx1 != -1) ? "yes" : "no");
            LWLockRelease(seg_array->lock);
            return false;
        }
    }
    else if (task_type == SEGMENT_UPDATE_REBUILD_FLAT || task_type == SEGMENT_UPDATE_REBUILD_DELETION)
    {
        ...
        if (segment_idx0 < 0 || segment_idx0 >= MAX_SEGMENTS_COUNT || segment_idx1 != -1)
        {
            // elog(DEBUG1, "claim_merge_task: invalid segment indices for rebuild operation - worker %d, lsm_idx = %d, segment_idx0 = %u (valid: %s), segment_idx1 = %u (should be -1: %s)", 
            //      worker_id, lsm_idx, segment_idx0, 
            //      (segment_idx0 >= 0 && segment_idx0 < MAX_SEGMENTS_COUNT) ? "yes" : "no",
            //      segment_idx1, (segment_idx1 == -1) ? "yes" : "no");
            LWLockRelease(seg_array->lock);
    ...
    {
        // elog(DEBUG1, "claim_merge_task: segment not in use - worker %d, lsm_idx = %d, segment_idx0 = %u (in_used: %s), segment_idx1 = %u (in_used: %s)", 
        //      worker_id, lsm_idx, segment_idx0, 
        //      seg_array->segments[segment_idx0].in_used ? "yes" : "no",
        //      segment_idx1, 
        //      (segment_idx1 != -1) ? (seg_array->segments[segment_idx1].in_used ? "yes" : "no") : "N/A");
        LWLockRelease(seg_array->lock);
        return false;
    }
    ...
    {
        // elog(DEBUG1, "claim_merge_task: segment already compacting - worker %d, lsm_idx = %d, segment_idx0 = %u (is_compacting: %s), segment_idx1 = %u (is_compacting: %s)", 
        //      worker_id, lsm_idx, segment_idx0, 
        //      seg_array->segments[segment_idx0].is_compacting ? "yes" : "no",
        //      segment_idx1, 
        //      (segment_idx1 != -1) ? (seg_array->segments[segment_idx1].is_compacting ? "yes" : "no") : "N/A");
        LWLockRelease(seg_array->lock);
        return false;
    }
...
lsm_merge_worker_main(Datum main_arg)
{
    ...
    while (!got_sigterm)
    {
        ...
        else
        {
            ...
            ResetLatch(MyLatch);
            WaitLatch(MyLatch, WL_LATCH_SET | WL_TIMEOUT | WL_EXIT_ON_PM_DEATH, 400, 0);
        }
    }
